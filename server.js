...
function scheduleForSubscriber(entry) {
  ...
  (entry.schedule || []).forEach(n => {
    ...
    if (delay <= 0 || delay > 7 * 24 * 3600 * 1000) return; // max 7 days
    const id = setTimeout(() => {
  ...
  console.log(`[SCHED] ${entry.timers.length} notifications scheduled for ${entry.subscription.endpoint.slice(-10)}`);
}

// ── Routes ──

// Health check (UptimeRobotはここをpingし続けてサーバーをスリープさせない)
app.get('/', (req, res) => {
